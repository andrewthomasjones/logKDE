...
y1<-logKDE::logdensity(x, from=0.0001, to=200, na.rm = T)$y
x1<-logKDE::logdensity(x, from=0.0001, to=200, na.rm = T)$x
x2<-stats::density(x, from=0.0001, to=200, na.rm = T)$x
y2<-stats::density(x, from=0.0001, to=200, na.rm = T)$y

plotframe<-data.frame(x1=x1, x2=x2,y1=y1,y2=y2)

p1<-ggplot(data=airquality, aes(x=Ozone))+geom_histogram(aes(y=..density..),binwidth=2.5, alpha=0.8)+theme_bw(base_size =18)
p1<-p1+ggtitle("Ozone concentration in New York \nMay to September 1973")+ylab("Frequency")+xlab("Parts per Billion")
p1<-p1+geom_line(data=plotframe, aes(x=x2, y=y2, colour="Standard density"), size=1.1)
p1<-p1+geom_line(data=plotframe, aes(x=x1, y=y1, colour="logKDE density"), size=1.1)
p1+scale_color_discrete(name="Density")+theme(legend.position= c(.7,.6))
